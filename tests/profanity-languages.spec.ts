...
describe("Languages", () => {
  describe("Multi-language support", () => {
    it("should detect and censor profanity in specified languages", () => {
      expect(profanity.exists("I like big asss and I cannot lie", ["en"])).to.be.true;
      expect(profanity.exists("Ich bin ein arschloch", ["de"])).to.be.true;
      expect(profanity.exists("I like big asss and ich bin ein arschloch", ["en", "de"])).to.be.true;

      expect(profanity.censor("I like big asss and I cannot lie", undefined, ["en"])).to.equal(
      ...
      expect(profanity.censor("I like big asss and ich bin ein arschloch", undefined, ["en", "de"])).to.equal(
    ...
    it("should detect and censor profanity in a sentence with multiple languages", () => {
      expect(profanity.exists("I like big asss and ich bin ein arschloch", ["en", "de"])).to.be.true;
      expect(profanity.exists("Je suis un arschloch and I like big asss", ["en", "de"])).to.be.true;

      expect(profanity.censor("I like big asss and ich bin ein arschloch", undefined, ["en", "de"])).to.equal(
        `I like big ${profanity.options.grawlix} and ich bin ein ${profanity.options.grawlix}`,
      );
      expect(profanity.censor("Je suis un arschloch and I like big asss", undefined, ["en", "de"])).to.equal(
    ...
    it("should throw an error when an invalid language is specified", () => {
      expect(() => profanity.exists("I like big asss and I cannot lie", ["en", "invalid"])).to.throw('Invalid language: "invalid"');
      expect(() => profanity.censor("I like big asss and I cannot lie", undefined, ["en", "invalid"])).to.throw('Invalid language: "invalid"');
    });

    it("should handle language codes case-insensitively", () => {
      expect(profanity.exists("I like big asss", ["EN"])).to.be.true;
      expect(profanity.exists("ich bin ein arschloch", ["De"])).to.be.true;

      expect(profanity.censor("I like big asss", undefined, ["EN"])).to.equal(`I like big ${profanity.options.grawlix}`);
      expect(profanity.censor("ich bin ein arschloch", undefined, ["De"])).to.equal(`ich bin ein ${profanity.options.grawlix}`);
    });
  });
  ...
  describe("Language options", () => {
    it("should use languages specified in options when an empty language array is provided", () => {
      ...
      expect(customProfanity.exists("I like big asss", [])).to.be.false;
      expect(customProfanity.exists("ich bin ein arschloch", [])).to.be.true;

      expect(customProfanity.censor("I like big asss", undefined, [])).to.equal("I like big asss");
    ...
    it("should use default language (en) when no languages are specified in options or method call", () => {
      ...
      expect(customProfanity.exists("I like big asss")).to.be.true;
      expect(customProfanity.exists("ich bin ein arschloch")).to.be.false;

      expect(customProfanity.censor("I like big asss")).to.equal(`I like big ${customProfanity.options.grawlix}`);
    ...
    it("should use provided languages even if options.languages is set", () => {
      const customProfanity = new Profanity({ languages: ["de"] });
      expect(customProfanity.exists("I like big asss", ["en"])).to.be.true;
      expect(customProfanity.exists("ich bin ein arschloch", ["en"])).to.be.false;

      expect(customProfanity.censor("I like big asss", undefined, ["en"])).to.equal(`I like big ${customProfanity.options.grawlix}`);
  ...
  describe("Word list management across languages", () => {
    ...
    it("should not detect or censor removed words across all languages", () => {
      const customProfanity = new Profanity();
      customProfanity.removeWords(["asss", "arschloch"]);
      expect(customProfanity.exists("I like big asss", ["en"])).to.be.false;
      expect(customProfanity.exists("ich bin ein arschloch", ["de"])).to.be.false;

      expect(customProfanity.censor("I like big asss", undefined, ["en"])).to.equal("I like big asss");
